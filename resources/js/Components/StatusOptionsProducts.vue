...
<template>
    <Menu as="div" class="relative inline-block">
        <div>
            <MenuButton :class="[statusStyles[product_status], as == 'button' ? 'shadow-sm text-sm px-4 py-2 focus:ring-2 focus:ring-offset-2' : 'text-sm px-2 py-1 focus:ring-1 focus:ring-offset-1', 'inline-flex justify-center w-full rounded-md border font-medium focus:outline-none']">
                <!-- {{ product_items && product_status ? product_items[product_status] : 'active' }} -->
                {{ product_items[product_status] }}
        ...
        <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <MenuItems :class="[position == 'top' ? 'bottom-10 mb-2 left-0' : 'mt-2 right-0', 'origin-top-right absolute w-40 rounded-md shadow-lg bg-white z-10 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 focus:outline-none']">
                <div class="py-1">
                    <MenuItem v-for="(title, item) in product_items" :key="item">
