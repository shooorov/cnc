...
const form = reactive({
	...
	total: props.item_inventory.total,
	document: props.item_inventory.document,
	group_items: props.item_inventory.items.length ? props.item_inventory.items : [{ ...blank_item }],
})

const blank_item = reactive({
	item_id: '',
	quantity: '',
	unit: '',
	rate: '',
...
const calculation = (index, input) => {
	...
	let selected_item = props.items.data.find(i => i.id == this_item.item_id);
	this_item.avg_rate = selected_item?.avg_rate ?? null;

	if (!this_item.unit) {
		this_item.unit = selected_item.unit;
	}
	form.total = form.group_items.reduce((carry, val) => carry + Number(val.total), 0);
}
...
const addItem = (length = 1) => {
	for (let i = 1; i <= length; i++) {
		let clone = { ...blank_item.value };
		clone.unit = '';
		form.group_items.push(clone);
	}
}
			...
			<div class="px-4 sm:px-6 lg:max-w-6xl lg:mx-auto lg:px-8">
				<div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
					...
					<div class="mt-6 h-9 flex space-x-3 md:mt-0 md:ml-4">
						<Link :href="route('item_inventory.create')"
				...
				<div class="bg-white shadow sm:rounded-lg">
					<div
						...
						<p class="max-w-2xl leading-10 text-gray-700 text-lg font-medium mb-4 sm:mb-0">{{
							string_change.item_inventory }} In Edit</p>

						<div class="flex-shrink-0 flex space-x-3">
							<Link v-if="navigation.routes.includes('item_inventory.compare')"
							...
							<Link v-if="navigation.routes.includes('item_inventory.document')"
								:href="route('item_inventory.document', item_inventory.id)"
								class="inline-flex items-center w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white hover:bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-primary-400">
						...
						<dl class="space-y-4 sm:space-y-6 px-5 py-6">
							...
							<div class="max-w-xl mx-auto">
								<div class="grid grid-cols-6 gap-6">
									<div class="col-span-6 sm:col-span-3">
										<label class="block text-sm font-medium text-gray-700"> Inventory Date <span
									...
									<div class="col-span-6 sm:col-span-3">
										<label class="block text-sm font-medium text-gray-700"> Total <span
												class="text-red-500">*</span> </label>
										<input v-model="form.total" type="text" placeholder="Total" readonly
							...
							<div class="relative">
								...
								<div class="relative flex justify-center ml-4"> <span
										class="px-3 bg-white text-lg font-medium text-gray-900"> {{ string_change.item_s }}
									</span> </div>
							</div>
								...
								<table class="table-auto sm:table-fixed min-w-full divide-y divide-gray-200">
									<thead class="bg-gray-50">
										<tr>
											<th scope="col"
												...
												#</th>
											<th scope="col"
												class="w-80 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
												{{ string_change.item }}</th>
											<th scope="col"
												class="w-56 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
												Quantity - Unit</th>
											<th scope="col"
												class="w-40 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
												Rate</th>
											<th scope="col"
												class="w-40 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
												Total</th>
											<th scope="col"
												class="w-24 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
												Avg Rate</th>
											<th scope="col"
												class="w-6 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
												#</th>
										</tr>
									</thead>
									...
									<tbody class="bg-white divide-y divide-gray-200">
										<tr v-for="(group_item, index) in form.group_items" :key="index">
											...
											<td>
												<Combobox class="" v-model="group_item.item_id" :items="items.data" />
											</td>

											<td>
												<div class="relative">
													<input v-model="group_item.quantity" @keyup="calculation(index, 'rate')"
													...
													<div class="absolute inset-y-0 right-0 flex items-center">
														...
														<select id="unit" v-model="group_item.unit"
															class="block w-full px-2 pr-6 focus:ring-indigo-400 focus:border-indigo-400 hover:bg-gray-100 focus:bg-transparent sm:text-sm border-gray-300 rounded">
															<option value="">Unit</option>
															<option v-for="name, key in units" :key="key" :value="key">{{
																key }}</option>
														</select>
													</div>
												</div>
											...
											<td>
												<input v-model="group_item.rate" @change="calculation(index, 'rate')"
													@keyup="calculation(index, 'rate')" placeholder="Rate" type="text"
													oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');"
													autocomplete="off"
											...
											<td>
												<input v-model="group_item.total" @change="calculation(index, 'total')"
													@keyup="calculation(index, 'total')" placeholder="Total" type="text"
													oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');"
													autocomplete="off"
													class="block w-full px-4 focus:ring-indigo-400 focus:border-indigo-400 hover:bg-gray-100 focus:bg-transparent sm:text-sm border-gray-300 rounded">
											</td>
											...
											<td>
												<input v-model="group_item.avg_rate" readonly placeholder="Avg Rate"
													type="text"
									...
									<tfoot class="bg-white">
										<tr>
                                            <th scope="col" class="py-2 text-left text-lg font-medium font-mono"></th>
											<th colspan="4" class="text-left pt-3">
												<button @click="addItem()" type="button"
													class="inline-flex items-center mr-3 px-4 py-2 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
													Add Item
													<PlusIcon class="ml-2 -mr-0.5 h-4 w-4" aria-hidden="true" />
												</button>
												<button @click="addItem(10)" type="button"
													class="inline-flex items-center mr-3 px-4 py-2 border border-transparent shadow-sm text-sm leading-4 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
													Add 10 Item
													<PlusIcon class="ml-2 -mr-0.5 h-4 w-4" aria-hidden="true" />
												</button>
											</th>
											<th scope="col" class="py-2 text-left text-lg font-medium font-mono"> {{
												Number(form.total).toFixed(2) }}</th>
										</tr>
									</tfoot>
								</table>
							</div>
