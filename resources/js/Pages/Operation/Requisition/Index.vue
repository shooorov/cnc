			...
			<div class="px-4 sm:px-6 lg:max-w-6xl lg:mx-auto lg:px-8">
				<div class="py-6 md:flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
					...
					<div class="mt-6 h-9 flex space-x-3 md:mt-0 md:ml-4">
						<Link :href="route('requisition.create')"
							...
							<tr v-for="(requisition, index) in requisitions" :key="requisition.id"
								...
								<td class="px-3 py-1 whitespace-wrap break-words">
									<div class="flex justify-end">
										<a :href="route('report.requisition', requisition.id)" target="_blank"
											class="text-gray-600 hover:text-gray-800 ml-3" title="print">
											<PrinterIcon class="w-6 h-6" aria-hidden="true" />
										</a>
										<Link :href="route('requisition.show', requisition.id)"
											class="text-primary-600 hover:text-primary-800 ml-3" title="detail">
										<ArrowTopRightOnSquareIcon class="w-6 h-6" aria-hidden="true" />
										</Link>
										<Link :href="route('requisition.edit', requisition.id)"
											class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
										<PencilSquareIcon class="w-6 h-6" aria-hidden="true" />
										</Link>
										<button @click="destroy(route('requisition.destroy', requisition.id))"
