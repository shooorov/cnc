                                    ...
                                    <table class="table-auto sm:table-fixed min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            <tr v-for="(group_item, index) in form.group_items" :key="index" class="divide-gray-200">
                                                ...
                                                <td class="px-1 py-2">
                                                    <div class="relative">
                                                        <input v-model="group_item.quantity" @change="calculation(index)" @keyup="calculation(index)" :readonly="!group_item.item_name" placeholder="Quantity" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" class="block w-full px-4 pr-12 focus:ring-primary-400 focus:border-primary-400 sm:text-sm border-gray-300 rounded-md">
  ...
  return {
    ...
    setup (props) {
        ...
        const form = reactive({
            ...
            group_items: props.product.data.group_items.length? props.product.data.group_items : [{
                id: '',
                item_id: '',
                quantity_use: null,
                unit_use: null,
            ...
            platter_items: props.product.data.platter_items?.length? props.product.data.platter_items : [{
                item_id: null,
                quantity: 1,
            }],
