			...
			<div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
				<div class="bg-white shadow sm:rounded-lg">
					<div
						...
						<div class="flex-1">
							<h3 class="text-base font-semibold leading-6 text-gray-900">Accounts</h3>
							<p class="mt-1 text-sm text-gray-500">
							</p>
						</div>
						<div class="flex-shrink-0 flex space-x-3">
							<Link :href="route('transaction.transfer')"
								class="inline-flex items-center w-full rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white hover:bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-primary-400">
						...
						<thead>
							<tr>
								...
								<th
									...
									Type</th>
								<th
									class="w-1/5 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">
									Account</th>
								<th
									class="px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-left">
									Branch</th>
								<th
									class="w-40 px-5 py-2 border-b bg-gray-100 text-xs font-bold uppercase tracking-wider text-right">
									Action</th>
							</tr>
						</thead>
						<tbody class="bg-white">
							<tr v-for="(account, index) in accounts" :key="account.id"
								...
								<td class="px-3 py-1 whitespace-wrap break-words">
									<div class="text-sm leading-5 font-semibold text-gray-700">{{ account.type }}</div>
								</td>

								<td class="px-3 py-1 whitespace-wrap break-words">
									<div class="flex items-center">
										<div class="">
											<div class="text-sm leading-5 text-gray-700">{{ account.name }}</div>
											<div class="text-sm leading-5 text-gray-700">{{ account.number }}</div>
										</div>
									</div>
								</td>

								<td class="px-3 py-1 whitespace-wrap break-words">
									<div class="text-sm leading-5 text-gray-600 font-semibold">{{ account.branch }}</div>
									<div class="text-sm leading-5 text-gray-600">{{ account.address }}</div>
								</td>

								<td class="px-3 py-1 whitespace-wrap break-words">
									<div class="flex justify-end">
										<Link :href="route('account.show', account.id)"
											class="text-primary-600 hover:text-primary-800 ml-3" title="detail">
										<ArrowTopRightOnSquareIcon class="w-6 h-6" aria-hidden="true" />
										</Link>
										<Link :href="route('account.edit', account.id)"
											class="text-indigo-600 hover:text-indigo-800 ml-3" title="edit">
										<PencilSquareIcon class="w-6 h-6" aria-hidden="true" />
										</Link>
										<button @click="destroy(route('account.destroy', account.id))"
